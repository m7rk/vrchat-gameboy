...
public class PPU : UdonSharpBehaviour
{
      private const int SCREEN_WIDTH = 160;
        ...
        // 8 4 0
        private Color[] color = new Color[] { Color.white, new Color(0.25f, 0.25f, 0.25f), Color.grey, Color.black};
        ...
        public void update(int cycles, MMU mmu) {
            ...
            if (isLCDEnabled(mmu.LCDC())) {
                ...
                if (mmu.LY() == mmu.LYC()) { //handle coincidence Flag
                    mmu.setSTAT(bitSet(2, mmu.STAT()));
                    if (isBit(6, mmu.STAT())) {
                        mmu.requestInterrupt(LCD_INTERRUPT);
                    }
                } else {
                mmu.setSTAT(bitClear(2, mmu.STAT()));
                }

            } else { //LCD Disabled
                scanlineCounter = 0;
                mmu.setLY(0);
                mmu.setSTAT((byte)(mmu.STAT() & ~0x3));
            }
        }
        ...
        private void renderBG(MMU mmu) {
            byte WX = (byte)(0xff & (mmu.WX() - 7)); //WX needs -7 Offset
            byte WY = mmu.WY();
            byte LY = mmu.LY();
            byte LCDC = mmu.LCDC();
            byte SCY = mmu.SCY();
            ...
            byte y = isWin ? (byte)(0xff & (LY - WY)) : (byte)(0xff & (LY + SCY));
        ...
        public void RenderFrame() {
            ...
            // force render texture to draw here.
            bmp.Render();
